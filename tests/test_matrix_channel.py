...
import PhyAgentOS.channels.matrix as matrix_module
from PhyAgentOS.bus.events import OutboundMessage
from PhyAgentOS.bus.queue import MessageBus
from PhyAgentOS.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from PhyAgentOS.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("PhyAgentOS.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "PhyAgentOS.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("PhyAgentOS.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "PhyAgentOS.channels.matrix.asyncio.create_task", _fake_create_task
    )
