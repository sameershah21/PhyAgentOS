...
from PhyAgentOS.channels.email import EmailChannel
from PhyAgentOS.config.schema import EmailConfig
...
def test_fetch_new_messages_parses_unseen_and_marks_seen(monkeypatch) -> None:
    ...
    fake = FakeIMAP()
    monkeypatch.setattr("PhyAgentOS.channels.email.imaplib.IMAP4_SSL", lambda _h, _p: fake)
...
async def test_send_uses_smtp_and_reply_subject(monkeypatch) -> None:
    ...
    monkeypatch.setattr("PhyAgentOS.channels.email.smtplib.SMTP", _smtp_factory)
